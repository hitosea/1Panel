...
func loadParams(param string) string {
	stdout, err := cmd.Execf("grep '^%s=' "+global.ExtendCtlPath+" | cut -d'=' -f2", param)
	if err != nil {
		panic(err)
	}
	info := strings.ReplaceAll(stdout, "\n", "")
	if len(info) == 0 || info == `""` {
		panic(fmt.Sprintf("error `%s` find in "+global.ExtendCtlPath, param))
	}
	return info
}

func loadChange() bool {
	stdout, err := cmd.Exec("grep '^CHANGE_USER_INFO=' " + global.ExtendCtlPath + " | cut -d'=' -f2")
	if err != nil {
		return false
	}
	return stdout == "true\n"
